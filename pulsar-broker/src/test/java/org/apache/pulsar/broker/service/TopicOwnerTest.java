...
public class TopicOwnerTest {
    ...
    private void spyZookeeperToDisconnectBeforeResponse(ZooKeeper zooKeeper, RequestMatcher matcher) {
    ...
    public void testAcquireOwnershipWithZookeeperDisconnectedAfterOwnershipNodeCreated() throws Exception {
        ...
        spyZookeeperToDisconnectBeforeResponse(zooKeeper1, request -> {
        ...
        try {
            ...
            pulsarAdmins[1].lookups().lookupTopic(topicName.toString());
        } catch (Exception ex) {
        ...
        CompletableFuture<Optional<Topic>> topicFuture = pulsar1.getBrokerService().getTopic(topicName.toString(), true);
        topicFuture.join();
    }
